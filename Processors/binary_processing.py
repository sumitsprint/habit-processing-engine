...
def build_behavior_context(raw_df, habit_name):
    ...
    if not active_df.empty:
        ...
        behavior_context["latest_active_entry"] = {
            "date": active_df.iloc[-1]["Date"].strftime("%d-%m-%Y"),
            "value": active_df.iloc[-1]["Value"]
        }
        ...
        while i < len(active_df):
            ...
            if row["Value"] != "SKIP":
                ...
                while (
                    j < len(active_df)
                    and active_df.iloc[j]["Value"] == "SKIP"
                ):
                    skip_count += 1
                    j += 1
                ...
                end_skip_date = active_df.iloc[j - 1]["Date"]

                # If j is still inside dataframe bounds,
                # a future decisive behavioral outcome exists.
                if j < len(active_df):

                    next_decisive_date = active_df.iloc[j]["Date"]
                    next_decisive_outcome = active_df.iloc[j]["Value"]
